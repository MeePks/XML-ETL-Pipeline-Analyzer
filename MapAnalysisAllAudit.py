...
#connect to the Database and collect RespectiveAuditName, servername,DBName

connection_string = f'DRIVER={{ODBC Driver 17 for SQL Server}};SERVER=GAD1PRCALOG001;DATABASE=RetailDPGroupReporting;Trusted_Connection=yes;'
connectioncnt = pyodbc.connect(connection_string)
cursorcnt = connectioncnt.cursor()
print("Sucessfully Connected to the Centralized Server")


querycnt = f"select *  FROM [dbo].[___RCALogTables] where auditName<>'RiteAid'"
cursorcnt.execute(querycnt)
centralizedrows = cursorcnt.fetchall()
for centralizedrow in centralizedrows:
    #Give the  AuditName, serverName and DatabaseName of the Audit where Tracking Maps Table is Available
    audit_name=centralizedrow.AuditName
    server_name=centralizedrow.ServerName #servername in which trackingmaps table is available
    db_name=centralizedrow.TrackingMapDatabaseName   #databse in which tracking Maps table is .
    TrackerLoadLogDB=centralizedrow.TrackerLoadlogDB
